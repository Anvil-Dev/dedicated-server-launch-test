...
The `1.21.7-neoforge` branch is a compatibility wrapper around the `universal` branch. It runs the universal action with Minecraft `1.21.7`, NeoForge, and loader version `21.7.25-beta`.
...
- name: Test mods on a dedicated server
  uses: Anvil-Dev/dedicated-server-launch-test@1.21.7-neoforge
  with:
    mods: |
      path/to/mod.jar
    quiet-setup: "true"
