...
The `1.16.3-fabric` branch is an exact-version compatibility wrapper around the `universal` branch. It runs the universal action with Minecraft `1.16.3`, Fabric, and Fabric Loader `0.18.4`.
...
- name: Test dedicated server launch
  uses: Anvil-Dev/dedicated-server-launch-test@1.16.3-fabric
  with:
    mods: |
      path/to/mod.jar
