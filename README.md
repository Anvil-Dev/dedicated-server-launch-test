...
The `1.16.3-forge` branch is a compatibility wrapper around the `universal` branch. It always runs Minecraft `1.16.3` with Forge `36.1.42` and forwards the inputs below to the universal action.
...
- name: Test mod on a dedicated server
  uses: Anvil-Dev/dedicated-server-launch-test@1.16.3-forge
  with:
    mods: |
      path/to/mod.jar
    quiet-setup: "true"
