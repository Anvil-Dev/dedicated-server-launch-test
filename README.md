...
The `26.1.2-neoforge` branch is a universal compatibility wrapper around the `universal` branch. It pins Minecraft `26.1.2`, the `neoforge` mod loader, and NeoForge loader version `26.1.2.94`, then forwards the supplied inputs to the universal action.
...
- name: Test dedicated server launch
  uses: Anvil-Dev/dedicated-server-launch-test@26.1.2-neoforge
  with:
    mods: |
      path/to/mod.jar
    extra-mods: |
