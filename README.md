...
The `1.21.4-neoforge` branch is a compatibility wrapper around the `universal` branch. It forwards all inputs while fixing the Minecraft version to `1.21.4`, the mod loader to `neoforge`, and the loader version to `21.4.157`.
...
- name: Test dedicated server launch
  uses: Anvil-Dev/dedicated-server-launch-test@1.21.4-neoforge
  with:
    mods: |
      path/to/mod.jar
