# Dedicated Server Launch Test for Minecraft 1.21.7 Forge

This exact-version branch is a compatibility wrapper around the [`universal`](https://github.com/Anvil-Dev/dedicated-server-launch-test/tree/universal) branch. It runs the universal action with Minecraft `1.21.7`, Forge, and loader version `57.0.3`.

## Inputs

- `mods` (recommended): The paths to the mod files, one path per line. Either `mods` or `mod` must be provided.
- `mod` (deprecated): Alias for `mods`, retained for compatibility with existing workflows.
...
- name: Test mods on a dedicated server
  uses: Anvil-Dev/dedicated-server-launch-test@1.21.7-forge
  with:
    mods: |
      path/to/mod.jar
    quiet-setup: "true"
