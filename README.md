...
The `1.21.10-neoforge` branch is a compatibility wrapper around the `universal` branch. It supplies the fixed Minecraft, mod loader, and loader versions for Minecraft 1.21.10 with NeoForge.
...
- name: Test dedicated server launch
  uses: Anvil-Dev/dedicated-server-launch-test@1.21.10-neoforge
  with:
    mods: |
      path/to/mod.jar
