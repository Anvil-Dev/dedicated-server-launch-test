...
The `1.21.4-fabric` branch is an exact-version compatibility wrapper around the `universal` action. It runs the universal action with Minecraft `1.21.4`, Fabric, and Fabric Loader `0.18.4`.
...
```yaml
- name: Test Fabric mod on a dedicated server
  uses: Anvil-Dev/dedicated-server-launch-test@1.21.4-fabric
  with:
    mods: |
      path/to/mod.jar
    quiet-setup: "true"
