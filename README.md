...
The `1.21.8-forge` branch is a compatibility wrapper around the `universal` branch. It delegates the test with Minecraft `1.21.8`, Forge, and Forge loader version `58.1.11` fixed for this branch.
...
- name: Test mods on a dedicated server
  uses: Anvil-Dev/dedicated-server-launch-test@1.21.8-forge
  with:
    mods: |
      path/to/mod.jar
