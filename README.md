...
The `1.21.3-neoforge` branch is a compatibility wrapper around the `universal` branch. It supplies the fixed Minecraft, mod loader, and loader versions while forwarding mod and setup inputs to the universal action.
...
- name: Test dedicated server launch
  uses: Anvil-Dev/dedicated-server-launch-test@1.21.3-neoforge
  with:
    mods: |
      path/to/mod.jar
    quiet-setup: "true"
