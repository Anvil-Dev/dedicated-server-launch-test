...
The `1.19.4-fabric` branch is an exact-version compatibility wrapper around the `universal` branch. It runs the universal action with Minecraft `1.19.4`, Fabric, and Fabric Loader `0.18.4`.
...
At least one of `mods` or the deprecated `mod` alias must be provided. Prefer `mods` for all new workflows.

## Example

```yaml
- name: Test dedicated server launch
  uses: Anvil-Dev/dedicated-server-launch-test@1.19.4-fabric
  with:
    mods: |
      path/to/mod.jar
