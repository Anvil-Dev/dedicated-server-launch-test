...
The `1.19.2-fabric` branch is an exact-version compatibility wrapper around the `universal` branch. It always runs Minecraft `1.19.2` with Fabric Loader `0.18.4` while forwarding mod and setup inputs to the universal action.
...
- name: Test mod on a dedicated server
  uses: Anvil-Dev/dedicated-server-launch-test@1.19.2-fabric
  with:
    mods: |
      path/to/mod.jar
