...
The `1.18.2-fabric` branch is an exact-version compatibility wrapper around the `universal` branch. It always runs Minecraft `1.18.2` with Fabric Loader `0.18.4`.
...
- name: Test mods on a dedicated server
  uses: Anvil-Dev/dedicated-server-launch-test@1.18.2-fabric
  with:
    mods: |
      path/to/mod.jar
    quiet-setup: "true"
