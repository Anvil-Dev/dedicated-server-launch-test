...
The `1.14.4-forge` branch is a compatibility wrapper around the `universal` branch. It always runs Minecraft `1.14.4` with Forge `28.2.28` while forwarding the inputs below to the universal action.
...
```yaml
- name: Test mods on Minecraft 1.14.4 Forge
  uses: Anvil-Dev/dedicated-server-launch-test@1.14.4-forge
  with:
    mods: |
      path/to/mod.jar
    ...
    other-files: |
      config@https://example.com/example-config.toml
    quiet-setup: "true"
```
