...
The `1.21.9-forge` branch is a compatibility wrapper around the `universal` branch. It runs the universal action with Minecraft `1.21.9`, Forge, and loader version `59.0.5`.
...
- name: Test dedicated server launch
  uses: Anvil-Dev/dedicated-server-launch-test@1.21.9-forge
  with:
    mods: |
      path/to/mod.jar
    quiet-setup: "true"
