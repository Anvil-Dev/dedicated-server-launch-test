...
The `1.21.8-fabric` branch is a compatibility wrapper around the `universal` branch. It supplies the fixed Minecraft, mod loader, and loader versions for Minecraft 1.21.8 with Fabric.
...
- name: Test dedicated server launch
  uses: Anvil-Dev/dedicated-server-launch-test@1.21.8-fabric
  with:
    mods: |
      path/to/mod.jar
    quiet-setup: "true"
